...
export default class SolenisReorderPageNew extends LightningElement {
    ...
    @track reorderRedirectflag=false;
    @track sortBy;
    @track openOrderList = [];
      ...
      async getMaterialDetail(){   
       ...
       // console.log('<<<Sold>>>>'+Sold);   
            if(this.SerachMaterial.length  > 2 || TempSerachMaterial.length>2){
               await ConMatrerialList({accountId: this.recordId, 
            ...
            if(this.SerachMaterial.length  > 2 || TempSerachMaterial.length>2){
               await MatrerialList({accountId: this.recordId, 
                            searchtext: TempSerachMaterial,
                            soldToId: Sold,
                            limitSize: this.rowLimit,
                            offset: this.rowOffSet
